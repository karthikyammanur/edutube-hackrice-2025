...
// Load env from outer project root, monorepo root, and from apps/api if present
...
function extractTopicsFromSummary(summary: string, maxTopics = 8): string[] {
  ...
  // Prefer bolded concept names like **Fourier Series** if present
  const boldRegex = /\*\*([^*]+?)\*\*/g;
  let bm: RegExpExecArray | null;
  while ((bm = boldRegex.exec(target))) {
    const candidate = bm[1].trim();
    if (candidate.length > 2 && !topics.includes(candidate)) topics.push(candidate);
